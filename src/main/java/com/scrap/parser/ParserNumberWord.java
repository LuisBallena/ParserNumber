...
public class ParserNumberWord {
        ...
        static {
            for (DecimalesEspeciales e : DecimalesEspeciales.values()) {
                if (byId.put(e.getId(), e) != null) {
                    throw new IllegalArgumentException("El id es duplicado: " + e.getId());
                }
            }
        }
    ...
    public static String convertDecimalToWord(Double nuumber){
        return convertDecimalToWord(nuumber,"CON",false);
    }

    public static String convertDecimalToWord(Double number, String prefixPuntoDecimal, boolean decimalConvertWord) {
        ...
        if (!decimal.equals("0")) {
            builder.append(String.format(" %s ", prefixPuntoDecimal));
            if (decimalConvertWord) {
                builder.append(convertNumberToWord(Integer.parseInt(decimal)));
            } else {
    ...
    public static String convertNumberToWord(Integer number) {
        ...
        convertDecenasToWord(decimal, unidades, builder);
    ...
    private static void convertDecenasToWord(int decimal, int unidades, StringBuilder builder) {
        switch (decimal) {
            case 1:
            case 2:
